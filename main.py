...
app = FastAPI(title="AgentPass - AI Agent Identity & Access Management", version="2.0.0")
...
async def pending_approvals():
    return auth_server.audit_logger.get_pending_approvals()


@app.post("/api/approvals/{task_id}/resolve")
async def resolve_approval(task_id: str, request: dict):
    approved = request.get("approved", False)
    result = auth_server.audit_logger.resolve_human_approval(task_id, approved)
    if approved:
        ws_broadcast("human_approval_result", {"task_id": task_id, "approved": True})
    else:
        ws_broadcast("human_approval_result", {"task_id": task_id, "approved": False})
    return result


@app.get("/api/security/alerts")
async def security_alerts(limit: int = 50, unacknowledged_only: bool = False):
    return auth_server.audit_logger.get_security_alerts(limit, unacknowledged_only)
...
async def demo_normal_delegation():
    ...
    step2 = {"step": 2, "action": "intent_parse", "description": "DocAgent解析意图", "intent": intent, "trace_id": trace_id}
    steps.append(step2)
    ...
    try:
        ...
        step3 = {"step": 3, "action": "token_issue", "description": "AuthServer签发Token(衰减层级0)", "token": token_result, "trace_id": trace_id}
        steps.append(step3)
        ...
        try:
            ...
            step5 = {"step": 5, "action": "mtls_verify", "description": "DataAgent提交mTLS签名验证", "delegation": delegate_result, "trace_id": trace_id}
            steps.append(step5)
            ws_broadcast("demo_step", step5)
            await asyncio.sleep(0.5)

            bitable_data = feishu_bitable.read_bitable("demo_app_token", "demo_table_id")
            step6 = {"step": 6, "action": "feishu_api", "description": "DataAgent调用飞书API返回数据", "data": bitable_data, "trace_id": trace_id}
            steps.append(step6)
...
async def demo_token_theft():
    ...
    step1 = {"step": 1, "action": "token_stolen", "description": "模拟Token被盗用场景", "trace_id": trace_id}
    steps.append(step1)
    ws_broadcast("demo_step", step1)
    await asyncio.sleep(0.5)

    fake_token = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJmYWtlIiwiYWdlbnRfaWQiOiJhZ2VudF9kb2NfMDAxIn0.fake_signature"

    step2 = {"step": 2, "action": "verify_attempt", "description": "使用盗用Token尝试验证", "trace_id": trace_id}
    steps.append(step2)
    ws_broadcast("demo_step", step2)
    await asyncio.sleep(0.5)
...
async def demo_injection_defense():
    ...
    step2 = {"step": 2, "action": "injection_scan", "description": "InjectionScanner三层检测", "scan_result": scan_result, "trace_id": trace_id}
    steps.append(step2)
    ws_broadcast("demo_step", step2)
    await asyncio.sleep(0.5)
    ...
    if scan_result["is_injection"]:
        ...
        step3 = {"step": 3, "action": "injection_blocked", "description": "返回PROMPT_INJECTION_BLOCKED错误码", "error_code": "PROMPT_INJECTION_BLOCKED", "threats": scan_result["threats"], "trace_id": trace_id}
        steps.append(step3)
        ws_broadcast("demo_step", step3)
...
async def demo_human_approval():
    ...
    step1 = {"step": 1, "action": "sensitive_request", "description": "DataAgent接收涉及敏感操作的委托(读取通讯录)", "trace_id": trace_id}
    steps.append(step1)
    ws_broadcast("demo_step", step1)
    await asyncio.sleep(0.5)
    ...
    try:
        ...
        try:
            ...
            if delegate_result.get("human_approval_required"):
                step2 = {"step": 2, "action": "approval_required", "description": "系统自动置为PENDING_HUMAN_APPROVAL", "task_id": delegate_result.get("task_id", ""), "trace_id": trace_id}
                steps.append(step2)
                ws_broadcast("demo_step", step2)
                await asyncio.sleep(0.5)

                step3 = {"step": 3, "action": "approval_popup", "description": "前端显示审批弹窗，等待30秒", "timeout": 30, "trace_id": trace_id}
                steps.append(step3)
                ws_broadcast("demo_step", step3)

                step4 = {"step": 4, "action": "approval_result", "description": "审批通过，签发5分钟单次使用Token", "delegation": delegate_result, "trace_id": trace_id}
                steps.append(step4)
                ws_broadcast("demo_step", step4)
            else:
...
async def demo_privilege_escalation():
    ...
    step1 = {"step": 1, "action": "escalation_attempt", "description": "Agent尝试请求超出注册基线的权限", "trace_id": trace_id}
    steps.append(step1)
    ws_broadcast("demo_step", step1)
    await asyncio.sleep(0.5)
...
<h3>委托边</h3>
<table>
<tr><th>源</th><th>目标</th><th>成功次数</th><th>拒绝次数</th><th>最近结果</th></tr>
{"".join(f'<tr><td>{e["source"]}</td><td>{e["target"]}</td><td>{e["success_count"]}</td><td>{e["deny_count"]}</td><td>{e["last_decision"]}</td></tr>' for e in graph["edges"]) if graph["edges"] else '<tr><td colspan="5">暂无委托记录</td></tr>'}
</table>
</div>
...
<div class="section">
<h2>安全告警</h2>
{"".join(f'<p class="alert">[{a["severity"].upper()}] {a["alert_type"]}: {a["message"]}</p>' for a in alerts) if alerts else '<p>暂无安全告警</p>'}
</div>

<div class="section">
<h2>演示追踪</h2>
<table>
<tr><th>Trace ID</th><th>开始时间</th><th>步骤数</th><th>涉及Agent</th><th>包含拒绝</th></tr>
{"".join(f'<tr><td>{t["trace_id"]}</td><td class="timestamp">{datetime.fromtimestamp(t["started_at"]).strftime("%H:%M:%S")}</td><td>{t["step_count"]}</td><td>{t["agents"]}</td><td>{"是" if t["has_deny"] else "否"}</td></tr>' for t in traces)}
</table>
</div>
